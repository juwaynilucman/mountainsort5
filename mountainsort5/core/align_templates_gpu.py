...
def compute_pairwise_optimal_offset(template1: torch.Tensor, template2: torch.Tensor):
    T = template1.shape[0]
    best_inner_product = -float('inf')
    best_offset = 0
    for offset in range(T):
        inner_product = torch.sum(torch.roll(template1, shifts=offset, dims=0) * template2).item()
...
def align_templates(templates: torch.Tensor):
    K = templates.shape[0]
    # T = templates.shape[1]
    # M = templates.shape[2]
    offsets = torch.zeros((K,), dtype=torch.int32, device=templates.device)
    pairwise_optimal_offsets = torch.zeros((K, K), dtype=torch.int32, device=templates.device)
    pairwise_inner_products = torch.zeros((K, K), dtype=torch.float32, device=templates.device)
    for k1 in range(K):
        for k2 in range(K):
    ...
    print('Align templates offsets: ', offsets_np)
    return offsets_np